...
double getCorCoef (double *X, double *Y, int size)
{
	...
	return fabs(size*a - b*c)/sqrt((b*b-size*d)*(c*c-size*e));
}

/**************************************************************
*Function gets the best score of correlation coefficients calculated
*from a hypothesis on a key byte
***************************************************************/

double update_cpa(double *vecHamingPoids, double **TracesxTimes)
{    
    double k;
    double corRel[NSAMP];
    int i;
    for (i=0; i<NSAMP; ++i)
    {
	    corRel[i] = getCorCoef(vecHamingPoids, TracesxTimes[i], NRES);
    }
    k=corRel[0];
...
/**************************************************************
*** Function gets the best guess out of 256 hypotheses on one key byte
***************************************************************/
int give_us_the_key(double * cpa){
...
/*
* Function calculates the Haming weight of a byte
*/
double HamingPoid (uint8_t v)
{
	double p=0;
	int i;
	for (i=0; i<8; ++i)
...
/*Function excuted by each thread
*/
void* thread_task(void* idx)
{
  ...
  int *idd = (int*)idx; 
  int id = *idd; 
  double vecHamingPoids[NRES];
  double cpa[0xff]={0};
  int j;
  int l,m=0;
  for(key[id]=0 ; key[id]<=0xff ; key[id]++)
  {   
    //for each hypothesis, calculate the vector of haming weight
    for(j=0 ; j<NRES ; j++){
    uint8_t sortie = Sbox[key[id] ^ result[j].plaintext[id]];
    vecHamingPoids[j] = HamingPoid(sortie);
  }

    // for each hypothesis, store its score in its corresponding case of cpa table
    cpa[key[id]]=update_cpa(vecHamingPoids, TracesxTimes);
    ++m;
    if (m==256)
      break;
  }
  int k =give_us_the_key(cpa);
  key[id]=k; 
  return NULL;
}
...
int main(int argc, char* argv[]){
  ...
  /************ File reading********/
	...
	int i,j;
	TracesxTimes = malloc (NSAMP*sizeof(double*));
	for (i=0; i<NSAMP; ++i)
	{
		TracesxTimes[i] = malloc(NRES*sizeof(double));
		for (j=0; j<NRES; ++j)
		{
			TracesxTimes[i][j] = result[j].datapoints[i];
		}
	}
